...
## Globals
Extensive use of global variables is used on purpose to keep the code as simple as possible.


function handle `u_actual_func = @(t) (...)` should be defined so that it uses the `mu_x` global variable to generate the output.
